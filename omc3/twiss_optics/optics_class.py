...
import tfs
from twiss_optics.twiss_functions import get_phase_advances, tau, dphi, get_all_rdts
...
class TwissOptics(object):
    ...
    def calc_chromatic_beating(self):
        ...
        LOG.debug(f"  Pk2Pk chromatic beating DBEATX: {res['DBEATX'].max() - res['DBEATX'].min()}")
        LOG.debug(f"  Pk2Pk chromatic beating DBEATY: {res['DBEATY'].max() - res['DBEATY'].min()}")
    ...
    def _check_k_columns(self, k_columns):
        ...
        for k in k_columns:
            if k not in self.twiss_df:
                LOG.debug(f"Added {k} with all zero to data-frame.")
                self.twiss_df[k] = 0.
